...
public class Cheeses {
	...
	private static final String[] sCheeseNames = {
		...
		"Vacherin-Fribourgeois", "Val de Loue", "Valencay", "Vasterbottenost", "Venaco", "Vendomois",
	...
	public Cheeses (String path) {
		if (path == null) {
			mDatabase = SQLiteDatabase.create(null); // memory-backed database
		} else {
			mDatabase = SQLiteDatabase.openDatabase(path, null, SQLiteDatabase.CREATE_IF_NECESSARY);
			mDatabase.execSQL("DROP TABLE IF EXISTS cheese");
		}
		mDatabase.execSQL("CREATE TABLE cheese (name TEXT, origin TEXT)");
		Log.i(TAG, "Cheese table created " + ((path==null)?"in memory":"on storage"));
	}
	
	private void beginTransaction(boolean oneTransaction) {
		if (oneTransaction) {
			mDatabase.beginTransaction();
		}
	}
	
	private void endTransaction(boolean oneTransaction) {
		if (oneTransaction) {
	...
	public void populateWithStringBuilder(boolean oneTransaction) {
		try {
			beginTransaction(oneTransaction);
			int i = 0;
			...
			mDatabase.setTransactionSuccessful(); // remember to call setTransactionSuccessful()
		} catch (Exception e) {
			System.out.println(e.toString());
		} finally {
			endTransaction(oneTransaction);
		}
	}
	...
	public void populateWithCompileStatement(boolean oneTransaction) {
		try {
			beginTransaction(oneTransaction);
			SQLiteStatement stmt = mDatabase.compileStatement("INSERT INTO cheese VALUES(?,?)");
			int i = 0;
			for (String name : sCheeseNames) {
				String origin = sCheeseOrigins[i++];
				stmt.clearBindings();
	...
	public void populateWithContentValues(boolean oneTransaction) {
		try {
			...
			for (String name : sCheeseNames) {
				...
				mDatabase.insert("cheese", null, values);
			}
			mDatabase.setTransactionSuccessful(); // remember to call setTransactionSuccessful()
		} catch (Exception e) {
			System.out.println(e.toString());
	...
	public void dump() {
		Cursor c = mDatabase.query("cheese", null, null, null, null, null, null);
		try {
	...
	private void iterateThroughAll (boolean all) {
		String[] columns = all ? null : new String[]{"name"};
		Cursor c = mDatabase.query("cheese", columns, null, null, null, null, null);
		if (c.moveToFirst()) {
			do {
				// nothing here, we just want to measure how long it takes to simply iterate through all rows
			} while (c.moveToNext());
		}
		c.close();
	}
	
	public void iterateAllColumns() {
		iterateThroughAll(true);
	}
	
	public void iterateFirstColumn() {
		iterateThroughAll(false);
	}
